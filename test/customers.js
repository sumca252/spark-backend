...
describe("/api/v1/graphql", () => {
    describe("Customers", () => {
        ...
        it("Should return a single customer by email", (done) => {
            chai.request(server)
                .get("/api/v1/graphql")
                .send({
                    query: `
                        {
                            getCustomerByEmail(email: "obrienwarren@example.com" ){
                                id,
                                user {
                                    id,
                                    first_name,
                ...
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.data.getCustomerByEmail.should.be.a("array");
                    res.body.data.getCustomerByEmail[0].should.have.property(
                        "id"
                    );
                    res.body.data.getCustomerByEmail[0].user.should.have.property(
                        "first_name"
                    );
                    res.body.data.getCustomerByEmail[0].user.should.have.property(
                        "last_name"
